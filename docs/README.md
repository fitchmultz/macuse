...
- The project-local pi extension keeps a persistent Codex app-server thread and exposes standalone read-only tools (`codex_cu_list_apps`, `codex_cu_get_app_state`) plus a sequence tool (`codex_cu_sequence`) with compact/minimal/full output, `targetScope: "main"` output filtering, sequence-level default `app`, running-app filtering, normalized text assertions, wait helpers, machine-readable parsed state/element/change details, partial failure results, stale-index guards, and robust element targeting (`element_index`, `element`, `elementId`, `elementDescription`, role/name selectors, or `arguments.targets` fallback objects).
