...
- Activity Monitor search/filter/clear and CPU/Memory tab probes have passed without destructive actions. Focus validation confirms the original frontmost app is restored. App approval defaults to `inherit`, matching Codex's Any App setting by auto-accepting app approvals in the bridge. Broader mutating GUI actions remain gated by `allowMutating` and a `safetyNote`.
