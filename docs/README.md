...
- The project-local pi extension keeps a persistent Codex app-server thread and exposes standalone read-only tools (`codex_cu_list_apps`, `codex_cu_get_app_state`) plus a sequence tool (`codex_cu_sequence`) with compact output, running-app filtering, partial failure results, and robust element targeting (`element_index`, `element`, `elementId`, or `elementDescription`).
