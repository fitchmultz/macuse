...
- Use `expectVisibleText` for UI-visible assertions; it matches substrings within parsed visible text nodes and exposed field/search/edit values. Use `expectText` only for app content text/value checks; it intentionally ignores macuse/upstream metadata such as CUA version headers.
