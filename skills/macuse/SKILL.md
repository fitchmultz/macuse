...
- If Computer Use times out or state looks stale, stop mutation and report the blocker. For stopped-session or transport-closed states, use `macuse_restart` or `/macuse-restart`; for timeouts, try a larger `toolTimeoutMs` or a read-only re-snapshot before considering another action.
