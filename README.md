...
Pi starts, resumes, and reloads with only `list_apps`, `get_app_state`, `macuse_sequence`, and `macuse_tools` active. Call `macuse_tools` with the exact direct, recording, history, or recovery tools needed; activated tools stay available until the next session boundary or reload.
