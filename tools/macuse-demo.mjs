...
async function main() {
  ...
  const resetSteps = [
    { label: 'Read Calculator before reset', tool: 'get_app_state', arguments: { app: 'Calculator' } },
    { label: 'Clear Calculator with Escape', tool: 'press_key', arguments: { app: 'Calculator', key: 'Escape' } },
    { label: 'Clear Calculator again with Escape', tool: 'press_key', arguments: { app: 'Calculator', key: 'Escape' } },
    { label: 'Verify reset state', tool: 'get_app_state', arguments: { app: 'Calculator' } },
  ];
  const reset = runJson('reset Calculator before demo assertions', ['tools/codex-computer-use-appserver.mjs', 'sequence', '--steps-json', JSON.stringify(resetSteps), '--allow-mutating', '--preserve-mouse', '--quiet', '--max-text-chars', '3000', '--tool-timeout-ms', String(opts.toolTimeoutMs)], { timeoutMs: opts.toolTimeoutMs + 60_000 });
  transcript.commands.push({ name: 'reset Calculator before demo assertions', command: reset.command, durationMs: reset.durationMs, result: reset.result });
