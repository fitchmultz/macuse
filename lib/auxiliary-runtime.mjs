  ...
  async #call(args, context) {
    ...
    for (;;) {
      ...
      try {
        ...
        context.missingTools = [`${server}/${tool}`];
        for (;;) {
          let cursor;
          do {
            const status = await this.#request('mcpServerStatus/list', { threadId: this.#threadId, detail: 'toolsAndAuthOnly', limit: 100, ...(cursor ? { cursor } : {}) }, context);
            if (status?.data?.some(entry => entry.name === server && Object.hasOwn(entry.tools ?? {}, tool))) {
              context.missingTools = [];
              break;
            }
