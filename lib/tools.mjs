...
const timeoutMs = { type: 'integer', description: 'Execution deadline in milliseconds, 1000-300000; default 90000. Cancellation never undoes an application action.' };

export const primaryTools = [
  {
    name: 'macuse',
    description: 'Run JavaScript in the persistent native macOS Computer Use runtime. First call: await cua.getState() or var app = await cua.getApp("Exact App"); read the emitted API docs and state before acting. Await every action. For accessibility Press use app.performSecondaryAction(index, "Press"); primary Press may be absent from Sky\'s secondary-action list. app.click(index) is pointer input; coordinates use returned screenshot pixels. getAXState(), getScreenshot(), and getAXStateAndScreenshot() emit observations automatically. Keep programs short; use JavaScript conditions, loops and assertions. Text output is capped; full state remains available for guards.',
    inputSchema: schema({
      ...
      safetyNote: text('Approved app, intended effect, and stop boundary. Never authorizes unrelated sends, purchases, deletes, credentials or privacy changes.'),
      timeoutMs,
  ...
  },
  {
    ...
    inputSchema: schema({
      ...
      safetyNote: text('Approved app, intended insertion, and stop boundary'),
