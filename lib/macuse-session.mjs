...
export class MacuseSession {
  ...
  callTool(tool, input = {}, options = {}) {
    ...
    if (input.timeoutMs < 1000 || input.timeoutMs > 300_000) return Promise.resolve(failure(tool, 'timeoutMs must be an integer from 1000 to 300000.'));
    ...
    const run = async () => {
      ...
      try {
        ...
        if (tool === 'macuse') result = await this.execute(input, { ...options, signal });
          ...
          let refresh;
          result = await insertText(this.native, input, this.runtime.getObservation(input.app), {
            signal,
            refreshObservation: async () => {
              const started = Date.now();
              try {
                refresh = await this.execute({ code: `await cua.getApp(${JSON.stringify(input.app)})`, trackFocus: false }, { signal });
                if (refresh.isError) throw new Error(refresh.content.filter(part => part.type === 'text').map(part => part.text).join('\n'));
                const observation = this.runtime.getObservation(input.app);
                if (!observation || observation.observedAt < started) throw new Error('A fresh full app snapshot is required before inserting text.');
                return observation;
              } finally {
                // This internal read must never authorize a later action, even if insertion refuses.
                this.runtime.invalidateObservation(input.app);
              }
            },
          });
          if (refresh) {
            result.details.macuse.refresh = refresh.details.macuse;
            result.details.macuse.kernelReset = refresh.details.macuse.kernelReset;
            if (refresh.details.macuse.interruption) result.details.macuse.interruption = refresh.details.macuse.interruption;
          }
