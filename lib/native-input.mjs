...
export async function insertText(native, input, observation, { signal, refreshObservation } = {}) {
	...
	try {
		...
		if (input.allowMutating !== true || typeof input.safetyNote !== "string" || !input.safetyNote.trim()) {
			throw new Error("insert_text requires allowMutating:true and a nonempty safetyNote naming the target, intended effect and stop boundary. These flags do not supply user permission.");
		}
