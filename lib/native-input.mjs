...
import { findSameElement } from './app-state.mjs';

// The parent serializes this entire preflight/edit/readback with its other GUI operations.
export async function insertText(native, input, observation, { signal } = {}) {
	...
	try {
		...
		const sameDocument = observation.url ? observation.url === window.document
			: observation.title != null && observation.title === window.title;
		if (!sameDocument) throw new Error("The focused window/document changed since the last observation. Observe the intended document again.");
