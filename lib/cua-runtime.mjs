...
export class CuaRuntime {
  ...
  async run(input, signal, onUpdate, generation) {
    ...
    try {
      ...
      // Completed guest errors include an execution duration, even when their text says "timeout".
      if (result.isError && result._meta?.["codex/nodeReplExecutionDurationMs"] === undefined
        && /timed?\s*out|timeout|kernel.{0,30}reset|reset.{0,30}kernel/i.test(text)) await this.interrupt("timeout");
      else {
