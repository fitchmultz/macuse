    ...
    const task = tail.then(async () => {
      ...
      try {
        ...
        if (input.element_index !== undefined) {
          ...
          if (!target || target.descendants !== original.descendants || target.disabled || target.name !== original.name || target.value !== original.value) fail("Target identity or value changed since the last public observation. Inspect a fresh state before acting.");
