...
export const GUI_METHODS = ["click", "drag", "paste", "perform_secondary_action", "press_key", "scroll", "select_text", "set_value", "type_text"];
const methods = new Set(["list_apps", "get_app_state", ...GUI_METHODS]);
const operationKeys = {
  get_app_state: ["app", "disableDiff"],
  click: ["app", "element_index", "x", "y", "mouse_button", "click_count"],
  drag: ["app", "from_x", "from_y", "to_x", "to_y"],
  paste: ["app", "text", "format"],
...
function appWideCommandKey(key) {
  const keys = key.toLowerCase().split("+").map(part => part.trim());
  const command = /^(?:super|meta)(?:_[lr])?$|^(?:cmd|command)$/;
  const modifiers = keys.slice(0, -1);
  // Save, Close, New, Open, Print, Quit, Hide and Minimize, including Shift/Option variants.
  if (modifiers.some(part => command.test(part))
    && modifiers.every(part => command.test(part) || /^(?:shift|alt)(?:_[lr])?$|^option$/.test(part))
    && ["s", "w", "n", "o", "p", "q", "h", "m"].includes(keys.at(-1))) return keys.at(-1);
}
...
export function validateEnvelope(input) {
  ...
  if (input.allowMutating !== undefined && typeof input.allowMutating !== "boolean") fail("allowMutating must be a boolean.");
  if (input.safetyNote !== undefined && typeof input.safetyNote !== "string") fail("safetyNote must be a string.");
  if (input.allowMutating && (!input.apps?.length || !nonempty(input.safetyNote))) {
    fail("Mutations require nonempty apps, allowMutating:true and a concrete nonempty safetyNote. Match apps exactly to the identifiers used in code.");
  }
}
...
function operation(request) {
  ...
  switch (method) {
    ...
    case "paste":
      string("text");
      if (!["text", "md", "html"].includes(input.format)) fail("paste format must be text, md or html.");
      break;
    case "type_text":
      string("text");
      if (/[^\x00-\x7f]/.test(input.text)) fail("Native typeText corrupts non-ASCII text. Use paste, the host selected-text insertion tool, or setValue for an intended full-field replacement.");
      break;
    case "select_text":
...
export function createGuard({ dispatch, context = () => globalThis.nodeRepl, diagnostic = () => {} }) {
  ...
  return function handleRpc(request) {
    ...
    const task = tail.then(async () => {
      ...
      let action;
      try {
        if (GUI_METHODS.includes(request.method)) {
        ...
        if (input.element_index !== undefined) {
          ...
          // Check cell values before dispatch; setValue can legitimately change its parent cell on readback.
          if (!target || target.cellValueContext !== original.cellValueContext || target.descendants !== original.descendants || target.disabled || target.name !== original.name || target.value !== original.value) fail("Target identity or value changed since the last public observation. Inspect a fresh state before acting.");
          input.element_index = Number(target.index);
          action.target = { index: target.index, id: target.id, role: target.role, name: target.name };
        }
        const commandKey = request.method === "press_key" ? appWideCommandKey(input.key) : undefined;
        if (["type_text", "paste"].includes(request.method) || request.method === "press_key" && before.focused && !commandKey) {
          const focused = before.focused && findSameElement(before.focused, before) && findSameElement(before.focused, fresh);
          if (!focused || focused.cellValueContext !== before.focused.cellValueContext
            || focused.descendants !== before.focused.descendants || focused.index !== fresh.focused?.index || focused.value !== before.focused.value || focused.disabled) fail("Focused field changed or is unavailable. Observe and verify the intended field before keyboard input.");
        }
