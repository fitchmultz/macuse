...
function usesFocusedField(key) {
  const keys = key.toLowerCase().split("+").map(part => part.trim());
  // App-wide Command shortcuts (e.g. Save) do not target the focused field.
  return !keys.some(part => ["super", "cmd", "command", "meta"].includes(part))
    || ["a", "c", "v", "x", "z", "left", "right", "up", "down", "home", "end", "backspace", "delete"].includes(keys.at(-1));
}
  ...
  return function handleRpc(request) {
    ...
    const task = tail.then(async () => {
      ...
      try {
        ...
        if (["type_text", "paste"].includes(request.method) || request.method === "press_key" && before.focused && usesFocusedField(input.key)) {
