    ...
    const task = tail.then(async () => {
      ...
      try {
        ...
        if (request.method === "type_text") {
          ...
          if (!focused || focused.descendants !== before.focused.descendants || focused.index !== fresh.focused?.index || focused.value !== before.focused.value || focused.disabled) fail("Focused field changed or is unavailable. Observe and verify the intended field before typing.");
        }
