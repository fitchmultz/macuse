...
export function parseAppState(app, text) {
  ...
  let current;
  const flush = () => {
    if (current) elements.push(parseElement(current.index, current.lines.join("\n")));
    current = undefined;
  };
  ...
  for (const line of tree.split("\n")) {
    ...
    const measurement = /^\s*\d+(?:\.\d+)?\s+(?:bytes?|[kmgtpe]?i?b)(?:\/s)?\s*$/i.test(line);
    const match = !measurement && line.match(/^\s*(\d+)\s+([\s\S]+)$/);
    if (match) {
      flush();
      current = { index: match[1], lines: [match[2]] };
