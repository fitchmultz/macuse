...
export function parseAppState(app, text) {
  ...
  const flush = () => {
    if (current) {
      const element = parseElement(current.index, current.lines.join("\n"));
      element.parentPath = JSON.stringify(parents.slice(0, current.depth).map(parent => [parent.role, parent.id, parent.name, parent.value, parent.url]));
      elements.push(element);
      parents[current.depth] = element;
      parents.length = current.depth + 1;
    }
    current = undefined;
  };
...
export function findSameElement(element, observation) {
  ...
  if (candidates.length === 1 && (element.id || candidates[0].parentPath === element.parentPath)) return candidates[0];
