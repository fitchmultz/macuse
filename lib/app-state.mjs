...
export function parseAppState(app, text) {
  ...
  const siblingContexts = children.map(nodes => {
    ...
    return {
      ...
      nonItems: fingerprint(nodes.filter(node => !itemRoles.has(elements[node].role)).map(node => subtree[node])),
    };
  });
  for (let i = 0; i < elements.length; i++) {
    const path = [];
    for (let child = i, parent = parents[i]; parent >= 0; child = parent, parent = parents[parent]) {
      ...
      const { left, right, nonRows, nonItems } = siblingContexts[parent];
      // Other rows and peer scroll-area items are independent of this target.
      const siblings = depths[parent] === 0 ? empty : elements[child].role === "row"
        ? nonRows : ancestor.role === "scroll area" && itemRoles.has(elements[child].role)
          ? nonItems : fingerprint([left[positions[child]], right[positions[child] + 1]]);
