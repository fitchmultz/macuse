...
export function parseAppState(app, text) {
  ...
  flush();
  const parents = [], stack = [], children = elements.map(() => []);
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    for (let child = i, parent = parents[i]; parent >= 0; child = parent, parent = parents[parent]) {
      ...
      // A row's Value may name the item; other ancestor Values include volatile scroll positions.
      const siblings = depths[parent] === 0 || elements[child].role === "row" ? [] : children[parent].filter(sibling => sibling !== child).map(sibling => subtree[sibling]);
      path.unshift([[ancestor.role, ancestor.id, ancestor.name, ancestor.role === "row" ? ancestor.value : undefined, ancestor.url], siblings]);
    }
