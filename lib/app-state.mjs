...
const itemRoles = new Set(["row", "group", "container"]);
...
export function parseAppState(app, text) {
  ...
  const tree = text.replace(/^<app_specific_instructions>[\s\S]*?<\/app_specific_instructions>\s*/, "");
  const lines = tree.split("\n");
  const closingTag = lines.findLastIndex(line => /^\s*<\/app_state>\s*$/.test(line));
  const focusLine = lines.findLastIndex((line, index) => /^The focused UI element is\s+\d+\s/.test(line)
    && lines.slice(index + 1).every(next => !next.trim() || /^\s*<\/app_state>\s*$/.test(next) || /^Selected text:/.test(next)));
  let wrapped = false;
  for (const [index, line] of lines.entries()) {
    if (!current && elements.length === 0 && /^\s*<app_state>\s*$/.test(line)) { wrapped = true; continue; }
    if (wrapped && index === closingTag) { flush(); wrapped = false; continue; }
    const footer = wrapped ? closingTag : focusLine < 0 ? lines.length : focusLine;
    const selectionTrailer = /^Selected text:/.test(line) && index < footer
      && lines.slice(index + 1, footer).every(next => !next.trim());
    if (index === focusLine || selectionTrailer
      || !current && /^\s*(?:Computer Use state|App=|Window:|Selected text:|Note:)/.test(line)) {
      flush();
      continue;
    }
  ...
  flush();
  const parents = [], stack = [], positions = [], children = elements.map(() => []);
  for (let i = 0; i < elements.length; i++) {
    stack.length = depths[i];
    parents[i] = stack.at(-1) ?? -1;
    if (parents[i] >= 0) {
      positions[i] = children[parents[i]].length;
  ...
  for (let i = elements.length - 1; i >= 0; i--) {
    const descendants = children[i].map(child => subtree[child]);
    elements[i].descendants = fingerprint(descendants);
    subtree[i] = fingerprint([identity(elements[i]), descendants]);
  }
  const empty = fingerprint([]);
  const siblingContexts = children.map(nodes => {
    const left = [empty], right = Array(nodes.length + 1);
    for (const node of nodes) left.push(fingerprint([left.at(-1), subtree[node]]));
    right[nodes.length] = empty;
    for (let j = nodes.length - 1; j >= 0; j--) right[j] = fingerprint([subtree[nodes[j]], right[j + 1]]);
    return {
      left, right,
      nonRows: fingerprint(nodes.filter(node => elements[node].role !== "row").map(node => subtree[node])),
    };
  });
  for (let i = 0; i < elements.length; i++) {
    const path = [];
    const cellValues = [];
    for (let child = i, parent = parents[i]; parent >= 0; child = parent, parent = parents[parent]) {
      const ancestor = elements[parent];
      // Prefix/suffix hashes exclude this branch without rescanning a large sibling list.
      const { left, right, nonRows } = siblingContexts[parent];
      // Other rows are independent; a group might instead be part of this item.
      const siblings = depths[parent] === 0 ? empty : elements[child].role === "row"
        ? nonRows : fingerprint([left[positions[child]], right[positions[child] + 1]]);
      if (ancestor.role === "cell") cellValues.unshift(ancestor.value);
      path.unshift([[ancestor.role, ancestor.id, ancestor.name, itemRoles.has(ancestor.role) ? ancestor.value : undefined, ancestor.url], siblings]);
    }
    elements[i].context = fingerprint(path);
    elements[i].cellValueContext = fingerprint(cellValues);
  }
  ...
  const focusedIndex = lines[focusLine]?.match(/^The focused UI element is\s+(\d+)\s/)?.[1];
