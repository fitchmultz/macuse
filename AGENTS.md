...
- Record & Replay starts and Computer History start/resume require exact user intent plus `allowRecording: true` and a non-empty safety note. `computer_history_update_settings` requires exact approval plus `allowPrivacyChange: true`, a safety note, and the complete `observation` settings object. Stop and pause operations are allowed without those flags.
