...
- Mutating GUI actions must use `codex_cu_sequence` or `tools/codex-computer-use-appserver.mjs sequence` with a narrow target, before/after state checks, `allowMutating: true`, and a safety note. In pi, prefer the persistent `codex_cu_sequence` tool over the CLI bridge. For element-targeted actions, prefer stable `elementId` values from `get_app_state`; `element_index` may be a string or number and `element` is an alias. If a sequence fails, inspect returned completed steps plus the failed-step diagnostic before retrying.
