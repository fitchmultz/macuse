...
- Read-only Computer Use probes are allowed: `list_apps`, `get_app_state`, `event_stream_status`, `skysight_status`, and `skysight_list_exclusions`; the auxiliary status/list calls can expose activity, artifact, and privacy metadata.
- Record & Replay or Skysight starts require exact user intent plus `allowRecording: true` and a non-empty safety note. Skysight exclusion updates require exact approval plus `allowPrivacyChange: true`, a safety note, and scope-specific arguments. Stop operations are allowed without those flags.
