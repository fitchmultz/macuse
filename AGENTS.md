...
- Map app identifiers only through identities returned by the native service; no fuzzy aliases. AX `Press` is a primary action and may be absent from Sky's secondary-action list; other secondary actions must be listed. Prefer AX actions, keys, intended full-field values, and element-targeted scroll. Pointer actions require `allowPointer:true`. Never warp the cursor or promise universal focus/input isolation; report unavailable coverage and unknown attribution.
- `macuse_insert_text` requires a fresh same-app focused-field observation. It replaces only `AXSelectedText` after identity/value/selection checks and verifies exact readback. No keyboard, clipboard, or fallback replay. Refresh after insertion. Raw `typeText` is ASCII-only; paste is disabled.
